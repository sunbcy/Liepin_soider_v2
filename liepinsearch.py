    ...
    def download_liepin_searchjob(self):
        ...
        for job_request in job_requests:
            ...
            while(currentPage<pageSize):
                self.self_payload={"data":{"mainSearchPcConditionForm":{"city":city,"dq":dq,"pubTime":pubTime,"currentPage":currentPage,"pageSize":pageSize,"key":Key,"suggestTag":suggestTag,"workYearCode":workYearCode,"compId":compId,"compName":compName,"compTag":compTag,"industry":industry,"salary":salary,"jobKind":jobKind,"compScale":compScale,"compKind":compKind,"compStage":compStage,"eduLevel":eduLevel}}}
           
                r=requests.post(self.url,headers=self.headers,json=self.self_payload,proxies=self.proxy)#,verify=False
                ...
                try:
                    ...
                    if usermail=='saintbcy@163.com':#抄送给自己
                        mailsender.sendMail('MacBot',[usermail],[],'【个人职位推荐x3】',mailsender.content(ALL_JOB_STR_MAIL))
                    else:
                        mailsender.sendMail('MacBot',[usermail],['626813534@qq.com'],'{user}【个人职位推荐x3】'.format(user=unpushed_username),mailsender.content(ALL_JOB_STR_MAIL))
                    session.close()
                    quit()
                for i in jobCardList:
                    ...
                    job_origin='猎聘'
                    job_recommendation=Job_Recommendation(dataInfo=dataInfo,dataParams=dataParams,job_title=job_title,job_salary=job_salary,job_dq=job_dq,job_requireWorkYears=job_requireWorkYears,job_requireEduLevel=job_requireEduLevel,job_labels=job_labels,job_link=job_link,jobId=jobId,topJob=str(topJob),job_advViewFlag=str(job_advViewFlag),job_dataPromId=job_dataPromId,recruiterName=recruiterName,recruiterTitle=recruiterTitle,recruiter_imId=recruiter_imId,recruiter_imUserType=recruiter_imUserType,recruiter_chatted=str(recruiter_chatted),recruiterId=recruiterId,recruiterPhoto=recruiterPhoto,compId=compId,compName=compName,compScale=compScale,compStage=compStage,compLogo=compLogo,compIndustry=compIndustry,comp_link=comp_link,job_intro_content=job_intro_content,jobKind=jobKind,job_refreshTime=job_refreshTime,job_origin=job_origin,username=username,usermail=usermail)
                    ...
                    if unpushed_jobs:
                        ...
                        JOB_STR_QIYE_WECHAT="""【{job_title} {job_salary}】\n{recruiterName} {recruiterTitle}\n{compName} {compScale} {compStage} {compIndustry}\n{jobKind} 发布时间:{job_refreshTime}\n{job_dq} {job_requireWorkYears} {job_requireEduLevel} {job_labels} \n\n{job_intro_content}\n\n""".format(job_title=unpushed_job_title,job_salary=unpushed_job_salary,job_dq=unpushed_job_dq,job_requireWorkYears=unpushed_job_requireWorkYears,job_requireEduLevel=unpushed_job_requireEduLevel,job_labels=unpushed_job_labels,recruiterName=unpushed_recruiterName,recruiterTitle=unpushed_recruiterTitle,compName=unpushed_compName,compScale=unpushed_compScale,compStage=unpushed_compStage,compIndustry=unpushed_compIndustry,job_intro_content=unpushed_job_intro_content,jobKind=unpushed_jobKind,job_refreshTime=unpushed_job_refreshTime)
                        JOB_STR_MAIL="""【{job_title} {job_salary}】<br>{recruiterName} {recruiterTitle}<br>{compName} {compScale} {compStage} {compIndustry}<br>{jobKind} 发布时间:{job_refreshTime}<br>{job_dq} {job_requireWorkYears} {job_requireEduLevel} {job_labels} <br><br>{job_intro_content}<br><br>""".format(job_title=unpushed_job_title,job_salary=unpushed_job_salary,job_dq=unpushed_job_dq,job_requireWorkYears=unpushed_job_requireWorkYears,job_requireEduLevel=unpushed_job_requireEduLevel,job_labels=unpushed_job_labels,recruiterName=unpushed_recruiterName,recruiterTitle=unpushed_recruiterTitle,compName=unpushed_compName,compScale=unpushed_compScale,compStage=unpushed_compStage,compIndustry=unpushed_compIndustry,job_intro_content=unpushed_job_intro_content,jobKind=unpushed_jobKind,job_refreshTime=unpushed_job_refreshTime)
                        print("""给用户 {username} 邮箱 {usermail} 推送以下信息:\n{job_recommendations_3}""".format(username=unpushed_username,usermail=unpushed_usermail,job_recommendations_3=JOB_STR_QIYE_WECHAT))
                ...
                currentPage+=1
                session.query(Job_Request).filter(Job_Request.id==user_id).update({Job_Request.currentPage:currentPage})#更新当前页
                session.commit()
            # break
